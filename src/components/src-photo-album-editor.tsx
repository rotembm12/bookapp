...
import './PhotoAlbumEditor.css'

// Placeholder components for UI elements (replace with your preferred UI library)
...
const useBookState = () => {
  ...
  const flipPage = (direction: 'next' | 'prev') => {
    if (direction === 'next' && currentPage < Math.ceil(images.length / 2) - 1) {
...
const ImageUploader = ({ onImageUpload }: { onImageUpload: (images: Image[]) => void }) => {
...
const ImageList = ({ images, onRemove }: { images: Image[], onRemove: (index: number) => void }) => {
  return (
  ...
  return (
    <div className="relative w-full h-[600px] bg-gray-100 rounded-lg shadow-lg overflow-hidden perspective">
      ...
      <div className="absolute bottom-4 left-4 right-4 flex justify-between">
        <Button onClick={() => onFlipPage('prev')} disabled={currentPage === 0 || !isOpen}>
          <ChevronLeft className="w-4 h-4 mr-2" />
          Previous
        </Button>
        <Button onClick={onToggleBook}>{isOpen ? 'Close Book' : 'Open Book'}</Button>
        <Button
...
export function SrcPhotoAlbumEditor() {
  const {
    images,
    currentPage,
    isOpen,
